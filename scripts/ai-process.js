...
function simpleCategory(article) {
  ...
  // 政策分类提高门槛：避免灾害/体育/外交等时政新闻被误归"政策"
  // 必须同时命中 (政策文种词) + (政策主体词或就业创业主题词)，才认定为政策
  // 政策文种：通知/办法/条例/意见/方案/规定/决定/公告/通告/实施细则/印发/若干政策/政策措施
  // 政策主体：国务院/部委/教育部/人社部/财政部/发改委/省政府/厅/局/办公室
  // 就业创业主题：就业/创业/高校毕业生/大学生/毕业生/职业/技能/人才/见习/灵活就业
  const titleZh = article.title
  const POLICY_DOC = /(通知|办法|条例|意见|方案|规定|决定|公告|通告|实施细则|印发|若干政策|政策措施|扶持政策|补贴政策|帮扶政策|三年行动|行动计划)/
  const POLICY_BODY = /(国务院|部委|教育部|人社部|财政部|发改委|省政府|省厅|市局|办公室|办公厅|部门|中央)/
  const EMP_TOPIC = /(就业|创业|高校毕业生|大学生|毕业生|职业|技能|人才|见习|灵活就业|就业帮扶|创业担保|创业孵化|就业补贴)/
  // 1) 标题含政策文种 + 政策主体 → 政策
  // 2) 标题含政策文种 + 就业创业主题 → 政策（大学生就业创业政策）
  // 3) 来源是部委/政策目录 → 政策（已在采集时标注，这里保留）
  if (POLICY_DOC.test(titleZh) && (POLICY_BODY.test(titleZh) || EMP_TOPIC.test(titleZh))) return '政策'
  if (/政策/.test(titleZh) && EMP_TOPIC.test(titleZh)) return '政策'
  if (article.category === '政策') return '政策'
  // 时政类新闻（含"时政/政务/政府"但未命中政策文种）不再归"政策"，归"综合"
...
分类规则（严格遵守）：
- "政策"：仅限政府/部门正式发布的政策文件，标题必须同时包含【政策文种词】(通知/办法/条例/意见/方案/规定/决定/公告/通告/实施细则/印发/若干政策/政策措施/扶持政策/补贴政策/帮扶政策/三年行动/行动计划) 和【政策主体或主题词】(国务院/部委/教育部/人社部/财政部/发改委/省政府/办公厅/就业/创业/高校毕业生/大学生/毕业生/职业/技能/人才/见习/灵活就业)。重点：大学生就业创业政策、高校毕业生就业政策、人社部就业创业政策、创业扶持政策、就业补贴政策都归"政策"。
- "就业"：招聘/求职/秋招/春招/校招/应届生求职/薪酬/职场等就业市场动态新闻（非政策文件）。
- "时政/外交/灾害/体育/社会民生/领导人活动/会议报道"等非政策文件，禁止归"政策"，按内容归"综合"或"社会"。
- 即使标题含"政府/时政/政务"但未命中政策文种词，不得归"政策"。
